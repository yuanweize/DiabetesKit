...
DiabetesKit 的结构化排版非常适合多模态大模型（如 GPT-4o, Claude 3.5 Sonnet, 豆包, 通义千问等）进行视觉分析。你可以将记录好的表格拍照发给 AI，并附上以下提示词（Prompt）：

> 请作为一位专业的内分泌科医生，分析这张血糖监测记录表。
> 1. 提取月度小结数据（平均值、最高值、最低值、低血糖次数）。
> 2. 分析血糖波动的规律（例如：是否存在固定的餐后高血糖或空腹高血糖）。
> 3. 结合表格底部的针对性参考范围，评估整体控制达标率。
> 4. 给出下阶段的日常饮食、运动或就医建议。请优先关注表格中可能出现的低血糖或紧急高血糖事件。
