...
**DiabetesKit** 是一个开源的**可打印健康管理工具集**，为糖尿病患者提供简洁、实用的日常追踪记录表。我们的目标是降低日常健康管理的门槛，特别关注老年用户的使用体验。目前已支持 **38 种语言**。

## 📥 快速下载

> **无需安装！** 下载 PDF，打印在 A4 纸上（竖向），即可让家人开始使用。
>
> 前往 [最新版本 Release 页面](https://github.com/yuanweize/DiabetesKit/releases/latest) — 所有 **38 个语言的 PDF** 均已附在资源列表中。
...
> **共 38 种语言** — 完整列表请查看 [Release 页面](https://github.com/yuanweize/DiabetesKit/releases/latest)。
...
make all        # 构建所有 38 种语言的 PDF（输出到 pdf/）
make list       # 显示所有自动发现的语言包
make zh_CN      # 构建单一语言（如简体中文）
make zh         # zh_CN 的快捷别名 (向后兼容)
make en         # en_US 的快捷别名 (向后兼容)
make clean      # 清理编译产物
```
...
- 🌍 **添加语言**：复制任意 `locales/*.json` 文件并翻译，放回 `locales/` 目录后运行 `make all` 即可 — **无需修改 Makefile！**
